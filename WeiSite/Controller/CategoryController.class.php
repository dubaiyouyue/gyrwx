...
// 			$data['ids'].="<a target='_self' href=$durl>删除</a>&nbsp;";
	...
	// public function del() {
	// 	$id = I ( 'id' );
	// 	$is_sub=M('weisite_category')->where(array('id'=>$id))->find();
	// 	if($is_sub['pid']==0){
	// 		$this->error ('父级栏目不能删除，建议修改！'); return false;
	// 	}else if(M('weisite_category')->where(array('pid'=>$id))->getField('id',true)){
	// 		$this->error ('该父级栏目有子栏目，请先删除子栏目！'); return false;
	// 	}else{
	// 		if(M('weisite_category')->where(array('id'=>$id))->delete()){
	// 			$this->success ( '删除成功'); 
	// 		}else{
	// 			$this->error ('删除失败');
	// 		}
